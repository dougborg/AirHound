...
docker-build-xiao: docker-image
    {{ _docker }} {{ image }} cargo build --no-default-features --features xiao --release --target {{ xiao_target }} {{ _build_std }}

# Build M5StickC firmware (in container)
[group('docker')]
docker-build-m5stickc: docker-image
    {{ _docker }} {{ image }} cargo build --no-default-features --features m5stickc --release --target {{ m5_target }} {{ _build_std }}
...
docker-check-xiao: docker-image
    {{ _docker }} {{ image }} cargo check --no-default-features --features xiao --release --target {{ xiao_target }} {{ _build_std }}

# Type-check M5StickC (in container)
[group('docker')]
docker-check-m5stickc: docker-image
    {{ _docker }} {{ image }} cargo check --no-default-features --features m5stickc --release --target {{ m5_target }} {{ _build_std }}

# Run library unit tests (in container)
[group('docker')]
docker-test: docker-image
    {{ _docker }} {{ image }} cargo test --lib --no-default-features

# Flash XIAO via container (Linux only — requires USB passthrough)
[group('docker')]
docker-flash-xiao: docker-image
    {{ _docker }} --device={{ device }} {{ image }} cargo run --no-default-features --features xiao --release --target {{ xiao_target }} {{ _build_std }}

# Flash M5StickC via container (Linux only — requires USB passthrough)
[group('docker')]
docker-flash-m5stickc: docker-image
    {{ _docker }} --device={{ device }} {{ image }} cargo run --no-default-features --features m5stickc --release --target {{ m5_target }} {{ _build_std }}
